...
public class player_controller : MonoBehaviour {
    ...
    void getKeyboardInput () {
        ...
        if (useMouseControls) {
            ...
            transform.position = Vector3.MoveTowards (transform.position, targetPosition, Time.deltaTime * playerSpeed * 2);
            return;
            // Vector2 distanceFromCenter = new Vector2 (playerTransform.position.x, playerTransform.position.y) - new Vector2 (mousePosition.x, mousePosition.y);
            // if (distanceFromCenter.y < 0f)
            //     keyPressedUp = true;
            // if (distanceFromCenter.y > 0f)
            //     keyPressedDown = true;
            // if (distanceFromCenter.x < 0f)
    ...
    void movePlayerBody () {
        ...
        for (int i = 0; i < playerBodySections.Count - 1; i++) {
            ...
            currentBodySection.transform.position = Vector3.Lerp (currentBodySection.transform.position, targetBodySection.transform.position, Time.deltaTime / playerSpeed * (playerSpeed * 20));
        }
    }
